...
import java.util.Calendar; // added for age calculation
import java.util.GregorianCalendar; // added for age calculation
import java.util.List;
import javax.swing.*;
import javax.swing.border.*;
import javax.swing.event.ChangeEvent; // added for DOB change
import javax.swing.event.ChangeListener; // added for DOB change
...
public class CensusMain {
    ...
    private void initialize() {
        ...
        fieldIdx = createSection(inputPanel, "Personal Information", new Color(255, 255, 210),
                new String[] { "Person ID", "First Name", "Last Name", "Gender", "Date of Birth", "Age",
                        "Marital Status", "Occupation", "Religion", "Nationality", "Ethnicity" },
                fieldIdx);
        fieldIdx = createSection(inputPanel, "Address Details", new Color(210, 240, 255),
                new String[] { "Address ID", "Region", "District", "Community", "Street Name", "House Number",
                        "GPS Location" },
                fieldIdx);
        ...
        setupAgeCalculation(); // initialize age calculation
    ...
    // ADDED: Helper method to set up age calculation
    private void setupAgeCalculation() {
        JComponent dobComp = fieldComponents.get("date of birth");
        JComponent ageComp = fieldComponents.get("age");

        if (dobComp instanceof JSpinner && ageComp instanceof JTextField) {
            JSpinner dobSpinner = (JSpinner) dobComp;
            JTextField ageField = (JTextField) ageComp;

            // Set initial value
            updateAgeFromDOB(dobSpinner, ageField);

            // Add change listener to update age when DOB changes
    ...
    // ADDED: Calculate and update age based on selected date of birth
    private void updateAgeFromDOB(JSpinner dobSpinner, JTextField ageField) {
        java.util.Date birthDate = (java.util.Date) dobSpinner.getValue();
        int age = calculateAge(birthDate);
        ageField.setText(String.valueOf(age));
    }

    // ADDED: Calculate age using calendar comparison
    ...
    private int createSection(JPanel parent, String title, Color bgColor, String[] labels, int fieldIdx) {
        ...
        for (String label : labels) {
            ...
            inputField = switch (label.toLowerCase()) {
                ...
                case "age" -> {
                    // replaced JComboBox with read-only JTextField
                    JTextField ageField = new JTextField();
                    ageField.setEditable(false); // Make read-only
                    ageField.setPreferredSize(new Dimension(250, 25));
                    textFields[fieldIdx] = ageField;
                    yield ageField;
                ...
                case "education level" ->
                    new JComboBox<>(new String[] { "None", "Primary", "JHS", "SHS", "Tertiary", "Postgraduate" });

                case "employment status" ->
                    new JComboBox<>(new String[] { "Employed", "Unemployed", "Self-Employed", "Student" });
